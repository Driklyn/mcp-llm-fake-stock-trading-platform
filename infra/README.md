...
```mermaid
flowchart TB
    V["viewers"] -->|"GET /api/v1/ticks/* + GET /api/v1/transactions/50 · 14s edge cache"| CF["CloudFront"]
...
| GET    | /api/v1/transactions/50         | trading-api     | recent combined ledger activity (trades, transfers, open orders), fixed 50-row window; query strings ignored; 14s edge cache       |
| GET    | /api/v1/portfolio               | trading-api     | account-only: cash, holdings, cost basis, realized gains, equity                                                                   |
...
| GET    | /api/v1/orders                  | trading-api     | list orders (open only); only callable via CloudFront header and 14s cache                                                         |
...
- `trading-api`'s read feed works the same way: `/api/v1/transactions/50` resolves a fixed 50-row window from the path and ignores
  `limit` query strings. CloudFront caches the feed at the edge (14s TTL via the
