 ...
 *   fixed-window ledger feeds /api/v1/trades/50 and /api/v1/transfers/50 (the
 *   14s edge cache shields the 1-RCU DynamoDB table and the DSQL ledger) and
 *   `VITE_API_BASE_URL` (HTTP API Gateway) for every dynamic route (portfolio,
 *   orders, transfers POST, assistant). Bypassing CloudFront on dynamic routes
 *   fixes CORS and enables zero-buffered LLM text streaming. No Node server
 *   required. WebSocket is unavailable through the CloudFront HTTP
 *   distribution, so the market price stays deterministic/local and the ledger
 *   polls every 15s.
 */
...
// GET /api/v1/ticks/* plus the fixed-window ledger feeds (/api/v1/trades/50,
// /api/v1/transfers/50) are served through the CloudFront edge (14s cache) to
// protect the 1-RCU DynamoDB table and the DSQL ledger. Only used in direct
// mode; proxy mode hits the Node server at the same localhost base. Falls back
// to apiBaseUrl when unset.
